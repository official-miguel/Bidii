 ...
 * Deterministic engine flow: pre-check → generate → validate → publish.
 * No AI generation. AI only translates scheduling preferences (done separately
 * on the Preferences page). This page runs the constraint solver.
 */
...
/**
 * Returns the URL to navigate to so the user can fix a specific issue type.
 * affectedClasses is passed for types where a classId query param is useful.
 */
function fixUrl(issue: PreCheckIssue): string | null {
  switch (issue.type) {
    ...
    case "EMPTY_SLOTS": {
      ...
      const classId = issue.affectedClasses?.[0];
      return classId
        ? `/principal/timetable/requirements?classId=${encodeURIComponent(classId)}`
        : "/principal/timetable/requirements";
    }
    case "INSUFFICIENT_CAPACITY":
      // Over-capacity → reduce requirements or extend template
      return "/principal/timetable/requirements";
    case "STREAM_IMBALANCE":
    case "NO_STUDENTS_IN_SUBJECT":
      return "/principal/students";
    case "CONFIGURATION_ERROR":
      return "/principal/timetable/template";
    default:
      return null;
  }
}
...
// ── Helpers ────────────────────────────────────────────────────────────────
function IssueRow({ issue, onNavigate }: { issue: PreCheckIssue; onNavigate: (url: string) => void }) {
              ...
              <div className={`rounded-xl border p-4 space-y-3
                ...
                {preCheck.issues.length > 0 && (
                  <div className="space-y-2">
                    {preCheck.issues.map((issue, i) => <IssueRow key={i} issue={issue} onNavigate={(url) => router.push(url)} />)}
                  </div>
                )}
              </div>
            )}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex gap-2 shrink-0 flex-wrap">
                  ...
                  {result.success && result.versionId && (
                    <button
                      type="button"
                      onClick={() => router.push(`/principal/timetable/builder?versionId=${result.versionId}`)}
                      className={`${secondaryButtonClass} text-xs`}
                    >
