...
export default function StatCard({ label, value, href, icon: Icon, color = "teal", sub, badge, badgeColor = "danger" }: StatCardProps) {
  ...
  const inner = (
    <div className="bg-card border border-line rounded-xl p-5 shadow-xs h-full
                    dark:bg-dark-surface dark:border-dark-border
                    hover:border-teal/40 hover:shadow-sm transition-all duration-150">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-slate dark:text-dark-muted truncate">
            {label}
          </p>
          <p className="text-3xl font-semibold text-ink dark:text-dark-text mt-1 leading-none">
            {value}
          </p>
          {sub && <p className="text-xs text-slate dark:text-dark-muted mt-1">{sub}</p>}
        </div>
        <div className="shrink-0 flex flex-col items-end gap-2">
          ...
          {badge && (
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full whitespace-nowrap ${badgeColorMap[badgeColor]}`}>
              {badge}
            </span>
          )}
        </div>
