 ...
 * src/lib/email.ts
 *
 * School email service — sends transactional email via the school's own
 * SMTP credentials (configured in Settings → Email (SMTP)).
 *
 * If the school has no EMAIL integration configured, all send functions
 * silently succeed (non-fatal) so callers don't need to guard.
 */
...
interface SmtpConfig {
  ...
  port: number;
  secure: boolean;
  user: string;       // usually the same as the "from" address
  password: string;   // the decrypted apiKey
  from: string;       // "School Name <email@domain>"
}

async function getSmtpConfig(schoolId: string): Promise<SmtpConfig | null> {
...
function createTransport(cfg: SmtpConfig) {
  return nodemailer.createTransport({
    ...
    tls: { rejectUnauthorized: false }, // works with self-signed certs
  });
}
...
export async function sendWelcomeEmail(payload: WelcomeEmailPayload): Promise<void> {
  const cfg = await getSmtpConfig(payload.schoolId);
  if (!cfg) return; // no email configured — silently skip
...
<style>
  ...
  .wrapper { max-width: 560px; margin: 40px auto; background: #FFFFFF; border-radius: 12px; border: 1px solid #E8EDF2; overflow: hidden; }
  ...
  .credentials h2 { margin: 0 0 14px; font-size: 12px; font-weight: 700; color: #2C7F7E; text-transform: uppercase; letter-spacing: 0.6px; }
  .cred-row { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; }
  .cred-row:last-child { margin-bottom: 0; }
  .cred-label { font-size: 12px; color: #667085; font-weight: 500; width: 100px; flex-shrink: 0; }
  ...
  .cta a { display: inline-block; background: #2C7F7E; color: #ffffff; text-decoration: none; font-size: 14px; font-weight: 600; padding: 12px 32px; border-radius: 8px; }
...
<div class="wrapper">
  ...
  <div class="body">
    ...
    <div class="notice">
      <p><strong>Important:</strong> This temporary password expires the moment you log in.
         You will be asked to create a permanent password before accessing any features.</p>
    </div>

    <ul class="steps">
      <li>Open the link below and sign in with your email and temporary password.</li>
...
IMPORTANT: This temporary password expires on first login. You will be
required to set a permanent password before accessing your dashboard.

Sign in here: ${v.loginUrl}

Steps:
1. Open the link above and enter your email and temporary password.
